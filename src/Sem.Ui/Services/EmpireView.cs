...
public sealed record EmpireChoice(string Key, string? Icon, EffectSet? Effects);
...
public sealed class EmpireView(DesignSession session, EmpireDesign design)
{
    ...
    private static EmpireChoice Chip(string key, string? icon, EffectSet? effects) =>
        new(key, icon, effects);
