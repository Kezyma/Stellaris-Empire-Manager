...
/// <remarks>
/// <para>
/// Held by the page and handed to every control that narrows or draws, so one set of filters
/// narrows them all together - which is the point of them: the question "who has Fanatic Purifiers"
/// is not a question about one of two lists.
/// </para>
/// <para>
/// Any within a heading by default, and all across them. Choosing two civics asks for rows with
/// either, and choosing a civic and an ethic asks for rows with both. That is what a set of
...
/// </para>
/// <para>
/// Its headings arrive at construction rather than being read off a static, which is the whole
/// reason it is generic: the wiki narrows civics by the same rules and there is nothing about any
/// of the above that is a fact about empires.
/// </para>
/// </remarks>
/// <typeparam name="TRow">What is being narrowed.</typeparam>
/// <param name="facets">The headings it answers for.</param>
/// <param name="text">Everything about a row that the search box should match on.</param>
public class Sifter<TRow>(IReadOnlyList<Facet<TRow>> facets, Func<TRow, string> text)
    where TRow : class
{
    /// <summary>Anything typed, matched against every word of the row that was typed rather than picked.</summary>
    ...
    /// <returns>The ticked keys, to be added to and removed from where they sit.</returns>
    ...
    /// <param name="facet">Which heading.</param>
    /// <returns>True when every tick has to be answered.</returns>
    ...
    /// <summary>Whether one row answers everything currently being asked.</summary>
    /// <param name="row">The row to judge.</param>
    /// <returns>True when it matches the search text and every heading's ticks.</returns>
    public bool Matches(TRow row)
    {
        ArgumentNullException.ThrowIfNull(row);

        if (Search.Length > 0 && !text(row).Contains(Search, StringComparison.CurrentCultureIgnoreCase))
        {
            return false;
        }
